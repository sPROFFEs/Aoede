...
class SpotifyService:
    ...
    async def search_item(
        self, client_id: str, client_secret: str, query: str, type: str = "track", limit: int = 1
    ) -> Optional[Dict]:
    ...
    async def search_tracks(
        self, client_id: str, client_secret: str, query: str, type: str = "track", limit: int = 10
    ) -> list:
    ...
    async def get_recommendations(
        ...
        try:
            ...
            for artist_id in artist_ids[:5]:  # Límite de 5 artistas
                ...
                if resp.status_code != 200:
                    logger.warning(
                        "Spotify top tracks failed for artist %s with status %s", artist_id, resp.status_code
                    )
                    continue
            ...
            if cache_path and items:
                ...
                with open(cache_path, "w") as f:
                    json.dump({"items": items, "expires_at": time.time() + (7 * 24 * 3600)}, f)
    ...
    async def get_new_releases(
        self, client_id: str, client_secret: str, country: str = "ES", limit: int = 12, cache_path: str = None
    ) -> list:
        ...
        try:
            ...
            with open(effective_cache, "w") as f:
                json.dump({"items": items, "expires_at": time.time() + expiration}, f)
    ...
    async def get_artist_albums(
        self, client_id: str, client_secret: str, artist_id: str, country: str = "ES", limit: int = 30
    ) -> list:
