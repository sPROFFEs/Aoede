                ...
                try:
                    ...
                    if container.status == "running":
                        ...
                        if ip_address and is_player_active(ip_address, user.username):
                            logger.info(
                                "Skipping idle container %s because user is still playing music", container_name
                            )
                            continue

                        logger.info(
                            "Stopping idle container %s after %s idle minutes",
                            container_name,
                            int(idle_duration.total_seconds() / 60),
                        )
                        container.stop()
                        reaped_count += 1
