...
def _migration_024_party_rooms(conn):
    conn.execute(
        text("""
    ...
    conn.execute(text("CREATE UNIQUE INDEX IF NOT EXISTS ix_party_rooms_owner_id ON party_rooms(owner_id)"))
    conn.execute(
        text("""
        CREATE TABLE IF NOT EXISTS party_room_queue_items (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def _migration_029_delete_reference_cleanup(conn):
    """Protect historical references on legacy SQLite schemas.

    Fresh databases receive equivalent foreign-key actions in their table
    definitions. Existing databases use triggers because rebuilding several
    populated tables is riskier than applying the same lifecycle policy in
    place. Party queues already cascade at the database layer; supported app
    deletion paths additionally repair their playback state before deletion.
    """
    conn.execute(
        text("""
    ...
    conn.execute(
        ...
        FOR EACH ROW
        BEGIN
            DELETE FROM user_favorites WHERE track_id = OLD.id;
            DELETE FROM playlist_items WHERE track_id = OLD.id;
            UPDATE playlists SET cover_track_id = NULL WHERE cover_track_id = OLD.id;
            UPDATE download_jobs SET resolved_track_id = NULL WHERE resolved_track_id = OLD.id;
            UPDATE track_delete_requests SET track_id = NULL WHERE track_id = OLD.id;
        END
        """)
    )
