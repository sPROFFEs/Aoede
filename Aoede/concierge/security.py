...
def purge_stale_attempts() -> int:
    ...
    now = time.time()
    stale = [
        ip for ip, rec in login_attempts.items() if rec["block_until"] <= now and (now - rec["last_seen"]) > RESET_TIME
    ]
    for ip in stale:
...
def is_trusted_proxy(address: str) -> bool:
    ...
    for configured in settings.trusted_proxy_ips:
        try:
            if candidate in ipaddress.ip_network(configured, strict=False):
                return True
        except ValueError:
            logger.warning("Ignored invalid trusted proxy network: %s", configured)
    return False


def validate_same_origin(request: Request) -> None:
    """Reject cross-origin cookie-authenticated state changes."""
    if request.method.upper() not in UNSAFE_METHODS or not request.cookies.get("access_token"):
        return
    ...
    if (urlparse(source).netloc or "").strip().lower() != expected_host:
...
def check_brute_force(request: Request):
    ...
    if record["block_until"] > now:
        ...
        raise HTTPException(
            status_code=429, detail=f"Too many attempts. IP blocked for security. Wait {int(remaining / 60)} minutes."
        )
