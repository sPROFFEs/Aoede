...
def test_new_party_room_schema_accepts_loading_status():
    ...
    assert ops_core.MIGRATIONS[-1][0] == "030_collaborative_playlists"
    engine.dispose()
...
def test_delete_reference_cleanup_migration_handles_legacy_foreign_keys():
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                "CREATE TABLE playlists (id INTEGER PRIMARY KEY, cover_track_id INTEGER, "
        ...
        conn.execute(
            text(
                ...
                "track_id INTEGER NOT NULL, FOREIGN KEY (playlist_id) REFERENCES playlists(id), "
                "FOREIGN KEY (track_id) REFERENCES tracks(id))"
            )
        )
        conn.execute(
            text(
                "CREATE TABLE download_jobs (id INTEGER PRIMARY KEY, target_modern_playlist_id INTEGER, "
                "resolved_track_id INTEGER, FOREIGN KEY (target_modern_playlist_id) REFERENCES playlists(id), "
                "FOREIGN KEY (resolved_track_id) REFERENCES tracks(id))"
            )
        )
        conn.execute(
            text(
                "CREATE TABLE user_favorites (id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, "
                "track_id INTEGER NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id), "
                "FOREIGN KEY (track_id) REFERENCES tracks(id))"
            )
        )
        conn.execute(
            text(
                "CREATE TABLE track_delete_requests (id INTEGER PRIMARY KEY, track_id INTEGER, "
                "FOREIGN KEY (track_id) REFERENCES tracks(id))"
            )
        )
        conn.execute(text("INSERT INTO users(id) VALUES (1)"))
