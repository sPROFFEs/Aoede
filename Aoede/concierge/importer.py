...
try:
    ...
    from database import DownloadSettings, SessionLocal, Track, User
except ImportError:  # pragma: no cover
    from concierge import cover_cache, metadata_service, track_identity
    from concierge.database import DownloadSettings, SessionLocal, Track, User
...
def extract_metadata(path: Path) -> LocalMetadata:
    ...
    try:
        ...
        if easy:
            ...
            md.genre = (easy.get("genre", [""])[0] or "").strip()
            date = (easy.get("date", easy.get("year", [""]))[0] or "").strip()
...
async def download_cover_if_missing(
    track_id: int,
    settings: DownloadSettings | None,
    title: str,
    artist: str,
    album: str,
...
def import_one(db, path: Path, *, dry_run: bool, stats: Stats) -> int | None:
    ...
    try:
        ...
        existing = track_identity.find_existing_track(db, file_hash=fhash, fingerprint=ident["fingerprint"])
        if existing:
            stats.duplicate += 1
            logger.info(
                "DUP   %s  →  already in DB as #%s (%s — %s)", path.name, existing.id, existing.artist, existing.title
            )
            if not dry_run:
        ...
        async def _process(tid: int):
            async with db_sem:
                ...
                try:
                    ...
                    if not t:
                        return
                    if await download_cover_if_missing(t.id, settings, t.title, t.artist, t.album, http_sem, http):
...
def get_api_settings(db) -> DownloadSettings | None:
    ...
    if not s:
        return None
    has_any = bool(getattr(s, "spotify_client_id", None) or getattr(s, "lastfm_api_key", None))
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.enrich and settings and stats.new_track_ids and not args.dry_run:
            logger.info("enriching %d new track(s) — covers + metadata cache", len(stats.new_track_ids))
            asyncio.run(enrich_pass(stats.new_track_ids, settings, workers=args.workers, stats=stats))
...
def main() -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="report what would happen without writing to disk or DB")
    parser.add_argument("--workers", type=int, default=4, help="concurrent API calls during enrichment (default 4)")
