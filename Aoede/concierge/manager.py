...
def provision_user_env(username: str):
    ...
    # Aoede and per-user Navidrome containers share UID 1000.
    dirs_to_create = [
        f"{user_dir}/data",
        f"{user_dir}/music",
...
def get_or_spawn_container(username: str):
    ...
    try:
        ...
        container = client.containers.run(
            ...
            name=container_name,
            detach=True,
            network="aoede-global",  # Ensure this network exists in your docker-compose
            environment={
            ...
            volumes={
                ...
                # User Playlists (Private, Read-Write) - Mounted so M3U files generated by Aoede are seen
                f"{settings.HOST_DATA_ROOT}/users/{username}/music/playlists": {
                    "bind": "/music/Playlists",
                    "mode": "rw",
                },
            },
        )
...
def get_dir_size(path):
    """Calcula el tamaño total de un directorio en bytes de forma recursiva, ignorando directorios inaccesibles."""
    total = 0
    try:
        ...
        logger.debug("Failed to measure disk usage for %s: %s", path, e)
    return total
